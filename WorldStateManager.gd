...
func restore_scene(scene_path: String) -> void:
	...
	for node in get_tree().get_nodes_in_group("world_item"):
		...
		for pos in removed_positions:
			...
			if pos is Vector2:
				...
				real_pos = Vector2(float(pos.get("x", 0)), float(pos.get("y", 0)))
			else:
				continue
			if node.global_position.distance_to(real_pos) < 4.0:
				node.queue_free()
				break
	...
	for data in state.get("items", []):
		...
		item.global_position = Vector2(float(data["x"]), float(data["y"]))


func mark_removed(scene_path: String, world_position: Vector2) -> void:
	if not scene_states.has(scene_path):
		scene_states[scene_path] = { "removed_positions": [], "items": [] }
	scene_states[scene_path]["removed_positions"].append({"x": world_position.x, "y": world_position.y})
...
func remove_dropped_item(scene_path: String, pos: Vector2) -> void:
	...
	for i in range(items.size() - 1, -1, -1):
		var item_pos := Vector2(float(items[i]["x"]), float(items[i]["y"]))
